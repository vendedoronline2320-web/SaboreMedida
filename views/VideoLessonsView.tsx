...
import { Play, Clock, ArrowLeft, Heart, FileText, Share2, Loader2, Tag, ChevronRight, Lock, Crown } from 'lucide-react';
...
const VideoLessonsView: React.FC<VideoLessonsViewProps> = ({ user, videos, favorites, onToggleFavorite, onRequireUpgrade, externalSelection, onSelectionHandled }) => {
  ...
  const handleSelect = async (video: VideoLesson) => {
    // Access Check
    const access = await db.checkPlanAccess('video', user, video);
    if (!access.hasAccess) {
      if (onRequireUpgrade) onRequireUpgrade();
      else alert('Esta aula é exclusiva para assinantes Premium.');
      return;
    }
            ...
            <div className="flex flex-col md:flex-row justify-between items-start gap-8 mb-12">
              <div className="space-y-4 max-w-3xl">
                <div className="flex flex-wrap items-center gap-3">
                  <span className="text-[10px] font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-widest bg-emerald-50 dark:bg-emerald-900/30 px-4 py-1.5 rounded-full">{selectedVideo.category || 'Geral'}</span>
                  {selectedVideo.isPremium && (
                    <span className="text-[10px] font-black text-amber-600 uppercase tracking-widest bg-amber-50 px-4 py-1.5 rounded-full flex items-center gap-1">
  ...
  return (
    <div className="animate-fade-in pb-24">
      <div className="mb-14 px-2 md:px-0">
              ...
              <div className="relative aspect-video overflow-hidden">
                ...
                <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                  <div className="w-16 h-16 bg-white/20 backdrop-blur-md border border-white/40 rounded-full flex items-center justify-center text-white shadow-2xl scale-75 group-hover:scale-100 transition-all duration-300">
                    {locked ? <Lock size={24} /> : <Play className="fill-current ml-1" size={24} />}
                  </div>
                </div>
                <div className="absolute top-5 left-5 flex flex-col gap-2">
                  <span className="bg-emerald-500 text-white px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest shadow-xl">{video.category || 'Geral'}</span>
                  {video.isPremium && (
                    <span className="bg-amber-500 text-white px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest shadow-xl flex items-center gap-1">
                      <Crown size={12} className="fill-current" /> Premium
                    </span>
                  )}
                </div>
                {locked && (
                  <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-[2px] flex items-center justify-center">
                    <div className="bg-white/90 p-4 rounded-3xl shadow-2xl flex flex-col items-center gap-2">
                      <Lock className="text-amber-500" size={32} />
                      <span className="text-[10px] font-black text-gray-900 uppercase">Upgrade Necessário</span>
                    </div>
                  </div>
                )}
              ...
              <div className="p-8 md:p-10">
                ...
                <div className="flex items-center justify-between pt-6 border-t border-gray-50 dark:border-slate-700 transition-colors">
                  ...
                  <div className={`font-black text-sm flex items-center gap-2 group-hover:gap-3 transition-all ${locked ? 'text-gray-300' : 'text-emerald-600 dark:text-emerald-400'}`}>
                    {locked ? 'Bloqueado' : 'Assistir Aula'} <ChevronRight size={18} />
                  </div>
                </div>
              </div>
            </div>
